// src/config/db.js — PostgreSQL database layer
'use strict';
...
async function createUser({ id, email, passwordHash, username, colorIndex }) {
  await pool.query(
    `INSERT INTO users (id, email, password_hash, username, color_index) VALUES ($1,$2,$3,$4,$5)`,
...
async function getAllVerifiedUsers() {
  const { rows } = await pool.query(`SELECT id, username, color_index FROM users WHERE verified=TRUE ORDER BY username`);
...
async function updatePlayerData({ id, username, colorIndex, coins, inventory, outfit }) {
  await pool.query(
    ...
    [username, colorIndex, coins, JSON.stringify(inventory||[]), JSON.stringify(outfit||{}), id]
  );
}
...
async function createVerifyToken(token, userId) {
  ...
  await pool.query(
    `INSERT INTO verify_tokens(token,user_id,expires_at) VALUES($1,$2,$3) ON CONFLICT(token) DO UPDATE SET user_id=$2,expires_at=$3`,
...
async function getAndDeleteVerifyToken(token) {
  ...
  const { rows } = await pool.query(`DELETE FROM verify_tokens WHERE token=$1 RETURNING *`, [token]);
  const row = rows[0];
...
async function getRoom(userId) {
  ...
  const r = rows[0];
  return { userId:r.user_id, roomName:r.room_name, wallpaper:r.wallpaper, flooring:r.flooring,
    placedItems: typeof r.placed_items==='string' ? JSON.parse(r.placed_items) : (r.placed_items||[]) };
}
...
async function saveRoom({ userId, roomName, wallpaper, flooring, placedItems }) {
  await pool.query(
    `INSERT INTO rooms(user_id,room_name,wallpaper,flooring,placed_items,updated_at)
     ...
     ON CONFLICT(user_id) DO UPDATE SET room_name=$2,wallpaper=$3,flooring=$4,placed_items=$5,
     updated_at=EXTRACT(EPOCH FROM NOW())::BIGINT`,
    [userId, (roomName||'My Room').substring(0,32), wallpaper||'default', flooring||'default', JSON.stringify(placedItems||[])]
  );
}

module.exports = {
  initSchema, createUser, getUserByEmail, getUserById, getAllVerifiedUsers,
